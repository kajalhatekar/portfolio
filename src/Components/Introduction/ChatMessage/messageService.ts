import { end, FAST_MODE_ID, messages, optionSelect } from "./messages.const";
...
class MessageService {
  ...
  onResponse(option: Option) {
    if (option.id === FAST_MODE_ID) this.fastMode = true;
  ...
  queueEndChat() {
    this.timeouts.push(
      window.setTimeout(() => this.endChat(), this.getTime(1500, 250)),
    );
  }
  ...
  shouldEndChat() {
    ...
    return optionSelect.content.every((option) =>
      [FAST_MODE_ID, ...this.selectedResponses].includes(option.id),
    );
  }
